...
test("PathContributions: template はちょうど 0 or 1。2 つは衝突エラー", () => {
  ...
  // 同名なら複数 profile が宣言してよい(create-only の複数トリガー用途)
  const sameName = new PathContributions("mise.toml", [
    { profile: "typescript", contribution: { template: "mise.toml.liquid" } },
    { profile: "terraform", contribution: { template: "mise.toml.liquid" } },
  ]);
  expect(sameName.templateName()).toBe("mise.toml.liquid");
});
