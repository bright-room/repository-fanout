...
```ts
type Axis = "languages" | "bundles";
listNames(axis: Axis): Promise<string[]>;      // <axis>/ 直下のディレクトリ名一覧
nameExists(axis: Axis, name: string): Promise<boolean>;
```
...
- core に `parseStrategyConfig(input: unknown)` を新設（object 検証・値検証。JSON パース不能もエラー）。
