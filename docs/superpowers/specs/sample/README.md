# テンプレ専用リポ サンプル構成（fragment + sync 戦略 / languages + bundles 2軸）
...
| **extends-field** | `renovate.json` | JSON パースして `extends` の管理エントリだけ更新 |

戦略はテンプレリポ**ルートの `strategies.json`**（配布先パス→戦略名のフラットな map。値は `extends-field` / `managed-block` のみ）で決まる。map に無いパスは replace、`seeds/**` は常に create-only。strategies.json が無い場合はエラー（黙って replace に降格させない）。新しいマージ意味論の追加は fanout のコード変更。

## 設計方針：languages + bundles の2軸（1 単位 = 1 自己完結ディレクトリ）

リポは **`languages`（構成言語の配列）** と **`bundles`（言語と独立な opt-in 束の配列。省略可）** を宣言する。マージ意味論は両軸とも同一（fragment + files を寄与）で、違いは分類だけ — `languages` には本物の言語のみを置き、oss のような opt-in 配布束は `bundles` に置く。言語名は **renovate-config の preset 名と 1:1 対応**（java / go / terraform / rust / typescript / kotlin …）。**framework 軸は持たない** — framework 対応（springBoot・Tauri 等）は renovate-config の言語 preset に内包済み（java.json / kotlin.json が `group:springBoot` を extends している）。
...
├── strategies.json                    # 配布先パス→戦略の map（上の表を参照）
...
│       └── files/
│           ├── CONTRIBUTING.md        #   公開リポ向け定型ドキュメント（replace）
│           └── SECURITY.md
└── examples/                          # （サンプル）出力例
...
将来 language ごとの合成が要るツールが出たら**キーを足すだけ**。配布ファイルを足すだけなら `files/` に置くだけ（fanout 改修不要）。**言語を増やす**のは `languages/<lang>/fragment.json` を1つ置くだけ（renovate-config に同名 preset を用意）。**束を増やす**のも同様に `bundles/<name>/` を置くだけ。

## renovate.json（extends-field）の描画ルール

1. **管理エントリの導出**：`extends 管理分 = base.renovate ++ (宣言 languages の renovate を宣言順に連結) ++ (宣言 bundles の renovate を宣言順に連結)`、順序保持 dedup。
2. **universe** = base ∪ **全 language ∪ 全 bundle**（宣言の有無に関わらず）の renovate 貢献の和集合 = 「fanout が管理するエントリの全集合」。
...
- ブロックの中身 = base テンプレ（`{{var}}` 置換）＋ language / bundle 貢献の合成（gitignore はセクションを連結。各セクションは見出しコメント + 無視パターン、セクション間は空行1つで区切る。無視パターンは横断で順序保持 dedup し、除去で空になったセクションは見出しごと省く）。
...
- **順序 = base → `languages` 配列の宣言順 → `bundles` 配列の宣言順**。貢献しない単位はスキップ。
- **重複エントリは先勝ち**で1つに。
- renovate の extends は上から順にマージし**後が前を上書き**（共通→言語固有→リポ独自の順）。
- **配布ファイル**（`languages/<lang>/files/` / `bundles/<name>/files/`）は、複数の単位が同じパスを出すと**衝突＝設定エラー**（1パス=1提供元）。
- **未知 language / bundle はエラー**（Terraform のタイポが黙って base のみ配布に劣化するのを防ぐ）。

## 補足

- **renovate-config は public 必須**（renovate の preset 解決のため）。common-files・renovate-config ともブランチ保護＋必須レビューを課す（設計 §3 ガバナンス）。
- 言語以外の opt-in 束は **`bundles` 軸**で宣言する（oss 等）。第3の軸は現時点では持たない。
