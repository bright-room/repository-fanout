...
2. **Terraform 拡張**：repository モジュールに `languages` / `fanout_vars` を追加、`fanout_manifest` output を生成（org + 個人 structure リポ）。on-merge で `terraform output` → fanout へ `POST /sync`。
