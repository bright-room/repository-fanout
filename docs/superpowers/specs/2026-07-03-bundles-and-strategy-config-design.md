...
- **キーのタイポは検出しない（既知の制限・意図的）**: キーが実在する配布先パスと一致しない場合、
  該当する本来のパスは既定の replace に降格する（例: `renovate.jsonn` とタイポすると renovate.json が
  replace になり、`{{renovate_extends}}` 未解決のテンプレ全文が PR として配布される）。配布は PR 経由で
  可視・テンプレリポはブランチ保護＋必須レビューで守られるため、孤児キー検証は実害が出るまで YAGNI。
  足す場合は base ∪ 全 language ∪ 全 bundle の files に対するキー照合で実装する。
- **extends-field を割り当てるパスの前提**: JSON ファイルであり、新規作成テンプレに
  `{{renovate_extends}}` を含むこと。既存ファイルは renovate JSON としてパースされる
  （パース不能なら該当リポ failed — 従来どおり）。
...
| strategies.json のキーが実在パスと不一致（タイポ） | 検出しない — 該当パスは replace 降格（PR で可視。テンプレリポのレビューで防ぐ。§3 既知の制限） |
