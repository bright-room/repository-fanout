...
- Workers プラン: `wrangler whoami` では判別不可(OAuth 認証は有効・アカウント bright-room)。**ダッシュボードでの確認をユーザーに依頼中**。状況証拠(child が CPU 35ms で完遂=Free の 10ms 超)は Paid を示唆。**P4 のデプロイ前までに確定必須**
