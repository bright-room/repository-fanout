...
- **望ましい状態（desired state）** = テンプレ専用リポの正本コンテンツ ＋ manifest（どのリポがどの言語か）。
...
| **structure リポ**（`bright-room/organization-structure`、`kukv/structure` ほか） | Terraform が repo→languages を宣言。on-merge で manifest を生成し fanout へ `POST /sync`（KV 保存＋reconcile 起動）。 |
...
親 Workflow（kick: on-demand。将来 Cron も）
  ...
  3. テンプレ専用リポから base/seeds/languages を読む（= fragment の素）
  4. 対象リポごとに子 Workflow を step.sleep を挟みつつ段階的に起動
        ▼
子 Workflow（1リポジトリ = 1インスタンス）
  - fragment を算出（base ∪ seeds ∪ 宣言 languages、{{var}} 置換・貢献合成）
...
9. language は **自己完結ディレクトリ**（`fragment.json` で非ファイル貢献を宣言＋`files/` で配布ファイル）。中央マップは持たない。renovate の extends は各 fragment.json の `renovate` を合成し、**管理エントリ（universe = base∪全 language の貢献）だけを入れ替え、リポが足した独自エントリは管理分の後ろに温存**（後勝ちでリポ優先）。
...
15. **未知 language はエラー**、CODEOWNERS 等は manifest の **ファイル単位 opt-out** 可（§3）。
...
flowchart TB
  ...
  subgraph SoT["望ましい状態 (Source of Truth)"]
    TR["テンプレ専用リポ<br/>base / seeds / languages"]
...
sequenceDiagram
  ...
  P->>TR: base/seeds/languages 読取 (chloe-chan)
  P->>C: repo x の子を起動
  C->>Repo: 実ファイル取得 (まだ無い)
