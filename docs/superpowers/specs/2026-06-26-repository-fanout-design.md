...
> ステータス: 設計確定（ブレスト合意済み）
> 最終更新: 2026-07-02（仕様変更: fragment + sync 戦略モデル。renovate は extends のみ管理・preset は renovate-config リポへ分離。.gitignore / CODEOWNERS は managed-block 化。宣言軸を profiles → **languages + bundles（2軸）** に変更・framework タグ廃止）
...
- **望ましい状態（desired state）** = テンプレ専用リポの正本コンテンツ ＋ manifest（どのリポがどの言語/束か）。
...
| **structure リポ**（`bright-room/organization-structure`、`kukv/structure` ほか） | Terraform が repo→languages/bundles を宣言。on-merge で manifest を生成し fanout へ `POST /sync`（KV 保存＋reconcile 起動）。 |
...
- `languages` = リポを構成する言語の配列（`terraform`, `typescript`, `java`, `kotlin`, `go`, `rust` …）。**renovate-config の preset 名と 1:1 対応**する軸。
...
**1 単位（language / bundle）= 1 自己完結ディレクトリ**。その単位の全貢献（配布ファイル＋renovate 等の合成貢献）を `languages/<lang>/`・`bundles/<name>/` に集約し、中央マップを持たない。ディレクトリ内の宣言ファイルは `fragment.json`（その単位が寄与する断片の宣言）。具体例は `docs/superpowers/specs/sample/` を参照。
...
- `{{renovate_extends}}` のみシステム導出変数（fragment.json から合成）、その他（`{{codeowner}}` 等）は manifest の `vars` 由来。
...
親 Workflow（kick: on-demand。将来 Cron も）
  ...
  3. テンプレ専用リポから base/seeds/languages/bundles を読む（= fragment の素）
  4. 対象リポごとに子 Workflow を step.sleep を挟みつつ段階的に起動
        ▼
子 Workflow（1リポジトリ = 1インスタンス）
  - fragment を算出（base ∪ seeds ∪ 宣言 languages ∪ 宣言 bundles、{{var}} 置換・貢献合成）
  - 対象リポの実ファイルを読み、**sync 戦略ごとにマージして望ましい内容を確定・比較**（§3）
    ...
    - extends-field（renovate extends）：実ファイルの extends だけ差し替えた全文と比較（他キー温存）
  - 差分ゼロ → no-op（PR を作らない）
...
- language/リポが manifest から外れても、過去に配ったファイルは**残置**される（掃除しない）。将来 cleanup モードを別途追加。
...
|---|---|---|---|
| `renovate.json` | extends-field | `extends` 配列の管理エントリのみ（他キー・リポ独自エントリ不可侵） | ✅ |
| `.github/CODEOWNERS` | managed-block | ブロック内の `* @{{codeowner}}` のみ（パス別ルールはリポ所有） | ✅ |
| `.github/release.yml` | replace | ファイル全体 | ✅ |
| `.gitignore` | managed-block | ブロック内（base＋language 行の合成）のみ | ✅ |

renovate preset 本体は **`bright-room/renovate-config`（構築済み）** に集約済み（`default.json` ＋ `java/go/terraform/rust/typescript/kotlin.json`）。common-files に preset は置かない。`languages/<lang>/files/`・`seeds/` は当面空でよく、必要が出た時点で追加する。
...
2. **Terraform 拡張**：repository モジュールに `languages` / `bundles` / `fanout_vars` を追加、`fanout_manifest` output を生成（org + 個人 structure リポ）。on-merge で `terraform output` → fanout へ `POST /sync`。
...
1. データモデルは **言語ベース（languages）+ opt-in 束（bundles）の2軸**（`languages` 宣言。言語名 = renovate-config preset 名と 1:1）。配布物は宣言から導出。framework 軸は持たない（renovate-config の言語 preset に内包）。renovate preset 本体は **`bright-room/renovate-config`** に分離（common-files は extends 参照文字列のみ保持）。
...
4. sync は **fragment + 戦略モデル**：パスごとの戦略レジストリ（replace / create-only / managed-block / extends-field）で反映方法が決まる。`seeds/**`=create-only は置き場所由来のまま。**リポ独自部分（ブロック外・extends 以外のキー・universe 外エントリ）は不可侵**。
...
9. language は **自己完結ディレクトリ**（`fragment.json` で非ファイル貢献を宣言＋`files/` で配布ファイル）。中央マップは持たない。renovate の extends は各 fragment.json の `renovate` を合成し、**管理エントリ（universe = base∪全 language∪全 bundle の貢献）だけを入れ替え、リポが足した独自エントリは管理分の後ろに温存**（後勝ちでリポ優先）。
...
15. **未知 language / bundle はエラー**、CODEOWNERS 等は manifest の **ファイル単位 opt-out** 可（§3）。
16. パス→戦略の割り当ては **common-files 直下の `strategies.json`（データ駆動）**、fragment 合成規則は core のコードで扱う（§3）。現状エントリ：`renovate.json`（extends-field）／`.gitignore`・`.github/CODEOWNERS`（managed-block）。`.gitignore` も MVP 対象。
...
flowchart TB
  ...
  subgraph SoT["望ましい状態 (Source of Truth)"]
    TR["テンプレ専用リポ<br/>base / seeds / languages / bundles"]
    RC["renovate-config リポ<br/>preset 本体 (public)"]
    SR["structure リポ x N<br/>Terraform (languages 宣言)"]
  end
...
| UC3 | 配布対象変更 | リポジトリ管理者 | 既存リポの `languages` 宣言を変更 → kick | 追加 language 分を配布 |
...
sequenceDiagram
  ...
  M->>SR: repository_x.tf 追加 (languages=[...]) を PR→merge
  ...
  P->>TR: base/seeds/languages/bundles 読取 (chloe-chan)
  P->>C: repo x の子を起動
  C->>Repo: 実ファイル取得 (まだ無い)
...
sequenceDiagram
  ...
  T->>TR: base/languages の fragment 編集 → PR→merge
...
sequenceDiagram
  ...
  M->>SR: 既存 repo の languages に "typescript" 追加 → merge
  ...
  P->>KV: manifest 読取
  P->>CHL: installation 列挙
  P->>TR: 正本 読取 (languages/typescript + 導出 preset)
  P->>C: 該当 repo の子を起動
  C->>Repo: 実ファイル取得
