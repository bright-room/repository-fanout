...
[warn] 送信側(structure/canonical-files)が OIDC 化される P2/P3 まで、この worker をデプロイすると旧 HMAC kick は 401 になる。デプロイ順序は P4 が管理する。
...
## 最終レビューと修正ラウンド記録(2026-07-04)

- 全体最終レビュー(独立レビュアー・Sonnet): **Critical 0 / Important 3 / Minor 4**。「削除の不変条件は end-to-end で保たれている。認可迂回・消しすぎ経路なし」との判定
- Important 3件+Minor 1件を即修正(impl-h、TDD):
  1. installation 照合の大文字小文字不整合 → 1ea25f5(照合のみ case-insensitive 化)
  2. readActualFiles が非404エラーを握りつぶし削除追従の記録が静かにロスト・retry 無効化 → af47b81(404のみスキップ。CLI 側 actualReader も同修正)
  3. 残置ファイルの Discord 通知(spec §5.7 の明記要件・実装漏れ) → 954206b(notifyKeptFiles。引き渡しは一度きりなので再通知しない)
  4. dist record の無変化書き込みスキップ(Free の KV write 予算節約) → 954206b
- Minor 対応: #5(global kick の ref 非検証)は「canonical-files リポ単位の信頼境界」として spec §6.2 に意図を明記。#6(エラーメッセージ文言の不統一)・#7(ウェーブはソフトキャップ)は実害なしのため記録のみ
- 修正後実測: core 114 / cli 9 / worker 62 = **185 tests 全パス**、typecheck 0、lint exit 0
