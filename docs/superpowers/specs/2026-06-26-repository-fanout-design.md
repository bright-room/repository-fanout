...
> ステータス: 設計確定（ブレスト合意済み）
> 最終更新: 2026-07-02（仕様変更: fragment + sync 戦略モデル。renovate は extends のみ管理・preset は renovate-config リポへ分離。.gitignore / CODEOWNERS は managed-block 化。宣言軸を profiles → **languages（単一軸）** に変更・framework タグ廃止）
...
1. データモデルは **言語ベース・単一軸**（`languages` 宣言。言語名 = renovate-config preset 名と 1:1）。配布物は宣言から導出。framework 軸は持たない（renovate-config の言語 preset に内包）。renovate preset 本体は **`bright-room/renovate-config`** に分離（common-files は extends 参照文字列のみ保持）。
