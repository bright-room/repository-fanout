...
// gitignore.liquid の正準形。canonical-files の templates/gitignore.liquid と同一で、
// 下の golden テストが描画結果(マーカー無し本文)を固定する。マーカーブロック差分による
// 全リポ一斉 PR を防ぐのが目的(旧 v2 renderGitignore との等価性は P-b で実測済み)。
